"""
DeepSeek Web for LunaTranslator
...
from translator.cdp_ws_client import (
    TranslationTask, CDPSession, format_prompt, clean_response,
    ensure_browser_launched, connect_to_tab, kill_browser,
    set_window_visibility, load_selectors, diagnose_selectors,
)
...
class TS(basetrans):
    ...
    def init(self):
        ...
        self.provider_name = self.selectors.get("name", "DeepSeek Web")
        self.profile_name = self.selectors.get("profile", "deepseek_profile")
    ...
    def _update_target_endpoint(self):
        """Update target URL and domain, supporting custom mirror endpoints."""
    ...
    def _send_message(self):
        ...
        time.sleep(0.12)
        # Resilient Enter fallback: if button click failed or text is still sitting in input, dispatch Enter
    ...
    def _do_translate(self, content, srclang_obj, tgtlang_obj):
        full_prompt = format_prompt(content, srclang_obj, tgtlang_obj)
